...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("🔄 [Next.js Proxy] Risk Prediction - Incoming request:", {
      url: request.url,
      method: request.method,
      payload: body
    });
    
    console.log("🚀 [Next.js Proxy] Forwarding to Python API:", `${PYTHON_API_BASE}/predict_risk`);
    
    const response = await fetch(`${PYTHON_API_BASE}/predict_risk`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      { error: 'Failed to connect to risk prediction service' },
      { status: 500 }
    );
  }
}
